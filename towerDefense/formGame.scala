...
object formGame {
  ...
  def initializeMap(map: LevelMap, coordsText: String) = {
    ...
    if (!pathIsConnected(coords)) throw new IOException("The input path is not connected! Check the input coordinates in gameInformation.txt.")
    if (!pathReachesEdges(coords)) throw new IOException("The input path does not reach the map edges! Check the input coordinates in gameInformation.txt.")
